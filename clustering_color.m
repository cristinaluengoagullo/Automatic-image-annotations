...
%        - img_seg(x, y, 3)  - 3D matrix with the number of the
...
function [vect_RGB] = clustering_color(he_filt, img_seg, lenght, width, x, y)
    for e=1:3
        seg_num=img_seg(x, y, e); % Current cluster being analysed    
    ...
    vect_RGB(1, 1) = mean(vect_pixel(1, :));
    vect_RGB(1, 2) = mean(vect_pixel(2, :));
    vect_RGB(1, 3) = mean(vect_pixel(3, :));
end